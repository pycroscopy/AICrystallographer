...
class ResSniffer(nn.Module):
    ...
    """
    def __init__(self, nb_filters=16, lrelu_a=0, with_logits=False):
        """
        Args:
        ...
        self.conv_in = nn.Sequential(
            nn.Conv2d(1, nb_filters, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(negative_slope=lrelu_a, inplace=True))
        self.conv_out = nn.Sequential(
            nn.Conv2d(nb_filters*2, nb_filters, kernel_size=3, stride=1, padding=1),
            nn.LeakyReLU(negative_slope=lrelu_a, inplace=True))
        self.conv_px = nn.Conv2d(nb_filters, 1, kernel_size=1, stride=1,
                                 padding=0)
        self.res_block1 = ResBlock(nb_filters, nb_filters*2, lrelu_a=lrelu_a)
        self.res_block2 = ResBlock(nb_filters*2, nb_filters*2, lrelu_a=lrelu_a)
